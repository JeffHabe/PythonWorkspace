...
# =============================================================================
        for i in range(interval,startPt-1,-1):
            ...
            if ((i-startPt)>min_time_interval+1):
                endPt=i+1
                delta=endPt-startPt
    ...
    #                 print(startPt)
    # =============================================================================
#                print(times[startPt].time)     
#                print(times[startPt])
                start.append(times[startPt])
                #print(times[startPt].hour)
                if(times[startPt].hour<6):
                    mrg+=1
                    R2_mrg.append(round(rsqRW,3))
                    mrgData.extend(ys_line)
                    OmrgData.extend(data[startPt:startPt+len(ys_line)])
                elif(times[startPt].hour>=6 and times[startPt].hour<12):
                    noon+=1
                    R2_noon.append(round(rsqRW,3))
                    noonData.extend(ys_line)
                    OnonData.extend(data[startPt:startPt+len(ys_line)])
                elif(times[startPt].hour>=12 and times[startPt].hour<=18):
                    aft+=1
                    R2_afr.append(round(rsqRW,3))
                    aftData.extend(ys_line)
                    OaftData.extend(data[startPt:startPt+len(ys_line)])
                elif(times[startPt].hour>=18 and times[startPt].hour<=23):
                    ngt+=1
                    R2_ngt.append(round(rsqRW,3))
                    ngtData.extend(ys_line)
                    OngtData.extend(data[startPt:startPt+len(ys_line)])
                if(endPt!=len(times)):    
...
#                 pltData += [
#                    go.Scatter(
#                        x=plotTime[startPt:endPt], # assign x as the dataframe column 'x'
#                        y=ys_line,
#                        mode='lines',
...
#     SnrCsvfileName='Data_csv\\SlidingWindow\\Chebyshev\\Mean\\Worst compressing data2019'+'\\'+fileName+"_DetailData.csv"
#     fT.mkfolder(SnrCsvfileName) 
...
# =============================================================================
        ...
        #print(len(fileName))
        for i in range(31,32):#len(fileName)
            #i=rndFile
            #print(fileName[i])
            sensorType=fileName[i][6:]  
            #print(sensorType)
            if(sensorType=='H1'or sensorType=='H2' or sensorType=='T1'or sensorType=='T2'  ):
                #print(fileName[i])
        ...
        MeanCsvfileName=dirMean+'\\CHEB(CoD)'+str(min_r2)+'wd'+str(wdsize_percent)+ssrType+dt.datetime.now().strftime('%m%d')+str(pIndex)+'.csv'
...
#        allR2rns=round(np.mean(dfpolyMeanDay['R2_R&S']),3)
        ...
        tolMeanFile='Data_csv\\SlidingWindow\\Chebyshev\\Mean\\CHEB(CoD)'+dt.datetime.now().strftime('%m%d')+'.csv'
        ...
        print('All is done in R2 : '+str(min_r2)+' wds : '+str(wdsize_percent)+' pIndex : '+ str(pIndex))
        pIndex-=1
