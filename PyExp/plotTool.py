...
def PlotLy(t,wdsize_percent,min_interval_percent,polyIndex,fileName,pltData,isOpen=False):
    ...
    filePath=".\Plot_html\\SlidingWindow\\"+'wdsize_percent='+str(wdsize_percent)+'\\min_interval_percent='+str(min_interval_percent)+'\\' 
    fT.mkfolder(filePath)
    fig=go.Figure(data=pltData,layout=layout)
    plot(fig,filename=filePath+fileName+".html",auto_open=isOpen)
